...
The helical geometry inherently possesses \textbf{Handedness}. A twist of $4\pi$ is required to restore the topological state of the helix relative to the background network.
\begin{itemize}
    \item \textbf{SU(2) Symmetry:} The excitation transforms under the double cover of the rotation group SO(3).
    \item \textbf{Phase Sign:} Rotating the helix by $2\pi$ introduces a phase factor of $-1$ ($e^{i\pi}$), requiring a $4\pi$ rotation to return to identity (+1). This provides a geometric origin for \textbf{Spin-\textonehalf\ statistics}.
\end{itemize}
...
% ----------------------------------------------------------------------
\section{Geometric Mass Generation (The Lepton Hierarchy)}
% ----------------------------------------------------------------------

Standard physics does not predict particle masses; it measures them. The Topological Stitching model proposes a geometric origin for mass based on the stability harmonics ($N$) derived in Section 5.

\subsection{The Mass Proxy Hypothesis}
The ``Rest Mass'' of a particle is proportional to the energy confined in its internal rotation. In our model, the rotational velocity component is $v_{rot} = \sqrt{c^2 - v_d^2}$. The total rotational action over one stitch cycle (N periods) is proportional to:
\begin{equation}
    M_N \propto N \cdot \sqrt{1 - \left(\frac{v_d(N)}{c}\right)^2}
\end{equation}

\subsection{Lepton Predictions}
Using the Electron ($N=3$) as the anchor, we search the spectrum for higher harmonics that match the known mass ratios of the Muon and Tau.

\begin{table}[h]
\centering
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{Particle} & \textbf{Mass (MeV)} & \textbf{Harmonic (N)} & \textbf{Predicted Ratio} & \textbf{Deviation} \\ \midrule
Electron ($e^-$) & 0.511 & \textbf{3} & 1.000 & (Anchor) \\
Muon ($\mu^-$) & 105.66 & \textbf{615} & 206.71 & -0.03\% \\
Tau ($\tau^-$) & 1776.86 & \textbf{10,345} & 3477.10 & -0.002\% \\ \bottomrule
\end{tabular}
\caption{Geometric prediction of Lepton Masses based on stability harmonics.}
\end{table}

\subsection{The Exclusion of N=1 and N=2 (Topological Stability)}
The spectrum also explains the lower bound of the lepton masses:
\begin{itemize}
    \item \textbf{N=1 (The Unknot):} The solution yields $M \approx 0$ and $v_{drift} \approx c$, corresponding to the Photon.
    \item \textbf{N=2 (The Unstable Digon):} The scaling law predicts a hypothetical mass of $\approx 335$ keV. However, no charged lepton exists at this energy. This absence is consistent with the topological basis of the theory: in knot theory, fewer than 3 crossings are insufficient to maintain a stable knotted structure (Reidemeister moves reduce them to the Unknot).
    \item \textbf{N=3 (The Trefoil):} The first topologically stable knot structure. This corresponds to the Electron, identifying it as the fundamental ground state of stable charged matter.
\end{itemize}

While this correlation may be coincidental, the high precision of the match suggests a potentially deep link between topological stability harmonics and the geometric origin of particle mass.

\begin{figure}[H]
    \centering
    % UNCOMMENT BELOW IF FILE EXISTS:
    \includegraphics[width=1.0\textwidth]{mass_hierarchy.png}
    \caption{\textbf{The Lepton Mass Hierarchy.} The blue curve shows the geometric mass proxy ($M \propto N \sqrt{1-v_d^2}$) for all harmonics. The Electron, Muon, and Tau fall precisely on integer values of $N$, suggesting a discrete geometric origin for the mass generation.}
    \label{fig:mass}
\end{figure}
...
Numerical analysis confirms that the Helical Stitch Ratio $R$ scales perfectly with Einstein's Lorentz Factor $\gamma$.
...
\subsection{Proposal B: Flow-Clock Orientation Test}
The Process-Universe model predicts that the internal phase-locking cycle of a particle depends not only on gravitational potential but also on the orientation of its internal rotation axis relative to the gravitational inflow. 

\textbf{Suppression in Current Clocks:} In most existing atomic clocks, the atomic states are isotropic or ensemble-averaged, which suppresses any directional signal. Therefore, current measurements (GPS, optical clocks) do not constrain this effect. A dedicated test requires a clock with a single well-defined internal axis (e.g., high-finesse optical cavity, single-ion with fixed quantization axis).

\textbf{Test Setup:} Two identical spin-polarized clocks at the same potential.
\begin{itemize}
    \item \textbf{Clock A:} Internal axis vertical (aligned with flow).
    \item \textbf{Clock B:} Internal axis horizontal (transverse to flow).
\end{itemize}

\textbf{Prediction:} The model predicts a fractional frequency shift:
\begin{equation}
    \frac{\Delta f}{f} = \alpha \left(\frac{v_{\text{flow}}}{c}\right) \left(\frac{v_{\text{rot}}}{c}\right)
\end{equation}
where $\alpha$ is a coupling constant determined by the degree to which the internal phase cycle couples to the clock transition. This constitutes a \textbf{distinctive} and falsifiable signature of the model.
...
\begin{enumerate}
    ...
    \item \textbf{Figure 4:} \texttt{mass\_spectrum.py} - Calculates the Lepton Mass Hierarchy harmonics.
    \item \textbf{Figure 5:} \texttt{drift\_spectrum\_plot.py} - Solves the transcendental stability equation for $N=1..20$.
\end{enumerate}

Repository URL: \url{https://github.com/your-repo/topological-stitching} \textit{(Placeholder)}
